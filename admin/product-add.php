...
if (isset($_POST['form1'])) {
	...
	// Validation
	$required_fields = ['tcat_id', 'mcat_id', 'ecat_id', 'lecat_id', 'p_name', 'p_current_price', 'p_qty'];
	...
	if ($valid == 1) {
		...
		try {
			...
			$statement = $pdo->prepare("INSERT INTO tbl_product (
                ...
                p_feature, p_condition, p_return_policy, p_total_view, p_is_featured, p_is_active, lecat_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			$statement->execute([
				...
				$_POST['p_return_policy'], 0, $_POST['p_is_featured'], $_POST['p_is_active'], $_POST['lecat_id']
			]);
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="mcat_id" class="form-control select2 mid-cat" id="mcat_id">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="ecat_id" class="form-control select2 end-cat" id="ecat_id">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="lecat_id" class="form-control select2 last-cat" id="lecat_id">
									<option value="">Select Last End Level Category</option>
									<!-- Options populated dynamically -->
								</select>
							</div>
						</div>
			...
			if (tcat_id) {
				$.ajax({
					...
					success: function(data) {
						...
						$('select[name="mcat_id"]').append('<option value="">Select Mid Level Category</option>');
						$.each(data, function(key, value) {
							$('select[name="mcat_id"]').append('<option value="' + value.mcat_id + '">' + value.mcat_name + '</option>');
						});
					}
				});
			} else {
				$('select[name="mcat_id"], select[name="ecat_id"], select[name="lecat_id"]').empty();
				$('select[name="mcat_id"]').append('<option value="">Select Mid Level Category</option>');
				$('select[name="ecat_id"]').append('<option value="">Select End Level Category</option>');
				$('select[name="lecat_id"]').append('<option value="">Select Last End Level Category</option>');
			}
		});

		// When mid category is changed
		$('select[name="mcat_id"]').on('change', function() {
			...
			if (mcat_id) {
				$.ajax({
					...
					dataType: 'json',
					success: function(data) {
						$('select[name="ecat_id"]').empty();
						$('select[name="ecat_id"]').append('<option value="">Select End Level Category</option>');
						$.each(data, function(key, value) {
							$('select[name="ecat_id"]').append('<option value="' + value.ecat_id + '">' + value.ecat_name + '</option>');
						});
					}
				});
			} else {
				$('select[name="ecat_id"], select[name="lecat_id"]').empty();
				$('select[name="ecat_id"]').append('<option value="">Select End Level Category</option>');
				$('select[name="lecat_id"]').append('<option value="">Select Last End Level Category</option>');
			}
		});

		// When end category is changed
		$('select[name="ecat_id"]').on('change', function() {
			var ecat_id = $(this).val();
			if (ecat_id) {
				$.ajax({
					url: 'get_last_category.php',
					type: 'POST',
					data: {
						ecat_id: ecat_id
					},
					dataType: 'json',
					success: function(data) {
						$('select[name="lecat_id"]').empty();
						$('select[name="lecat_id"]').append('<option value="">Select Last End Level Category</option>');
						$.each(data, function(key, value) {
							$('select[name="lecat_id"]').append('<option value="' + value.lecat_id + '">' + value.lecat_name + '</option>');
						});
					}
				});
			} else {
